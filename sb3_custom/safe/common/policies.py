...
from stable_baselines3.common.policies import BaseModel, BasePolicy, ActorCriticPolicy


import os
import sys
sys.path.append(os.path.dirname(os.path.abspath(os.path.dirname(__file__))))
from safe.common.torch_layers import create_mlp
from safe.common.distributions import (
...
class CustomActorCriticPolicy(ActorCriticPolicy):
    ...
    def _get_latent(self, obs: th.Tensor) -> Tuple[th.Tensor, th.Tensor, th.Tensor]:
        """
        지연 코드를 가져옴 (즉, 각 네트워크의 마지막 레이어의 활성화) 
        다른 네트워크에 대한 지연 코드를 가져옴
